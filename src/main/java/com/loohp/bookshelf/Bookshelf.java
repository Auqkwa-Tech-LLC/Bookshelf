...
public class Bookshelf extends JavaPlugin {
    ...
    public static final ConcurrentHashMap<String, Inventory> keyToContentMapping = new ConcurrentHashMap<>();
    public static final ConcurrentHashMap<Inventory, String> contentToKeyMapping = new ConcurrentHashMap<>();

    public static final ConcurrentLinkedQueue<String> bookshelfSavePending = new ConcurrentLinkedQueue<>();
    public static final ConcurrentLinkedQueue<Chunk> bookshelfLoadPending = new ConcurrentLinkedQueue<>();
    public static final ConcurrentLinkedQueue<Chunk> bookshelfRemovePending = new ConcurrentLinkedQueue<>();

    public static final ConcurrentHashMap<Player, BlockFace> lastBlockFace = new ConcurrentHashMap<>();

    public static final ConcurrentHashMap<Player, LWCRequestOpenData> requestOpen = new ConcurrentHashMap<>();
    ...
    public static final Set<String> isEmittingParticle = new HashSet<>();

    public static ConcurrentHashMap<Long, Location> tempRedstone = new ConcurrentHashMap<>();

    public static final ConcurrentHashMap<Player, Long> enchantSeed = new ConcurrentHashMap<>();
    ...
    public void onDisable() {
        ...
        Set<String> save = new HashSet<>(bookshelfSavePending);
        for (String key : save) {
    ...
    public static void loadBookshelf(World world) {
        List<World> worlds = new ArrayList<>(1);
        worlds.add(world);
    ...
    public static void loadBookshelfProgress() {
        CompletableFuture.runAsync(() -> {
            ...
            long lastDone = 0;
            while (done < spawnchunks && thisWorld.equalsIgnoreCase(currentWorld)) {
                Bukkit.getConsoleSender().sendMessage("[Bookshelf] Preparing bookshelves in spawn chunks in " + currentWorld + ": " + Math.round(((double) done / (double) spawnchunks) * 100) + "%");
